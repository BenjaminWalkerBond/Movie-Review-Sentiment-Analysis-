...
public class SentimentAnalysisApp {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int posCount1 = movieRev.classifyCollection(posFiles, ReviewScore.POSITIVE);
        int posCount2 = movieRev.classifyCollection(negFiles, ReviewScore.NEGATIVE);

        //NOTE: display results always returns the number of positive classifications
        displayResults(posCount1, posFiles.size(), ReviewScore.POSITIVE);
        displayResults(posCount2, negFiles.size(), ReviewScore.NEGATIVE);
        return;
    }
     ...
     */
    public static void displayResults(int posClassify, int size, ReviewScore score) {

        String label = (score.equals(ReviewScore.POSITIVE)) ? "positive" : "negative";
